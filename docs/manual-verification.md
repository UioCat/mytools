...
文字排版自动化命令：`swift test --filter 'ScreenshotAnnotationTests|ScreenshotEditorInteractionTests|ScreenshotRendererTests'`；完整测试使用下文统一命令。保留修复前 Return 后高度不变、修复后空行高度增长的失败与通过证据。原生文本视图测试覆盖组合文字、连续选词和换行，不替代真实输入法候选窗与打包应用实测。
