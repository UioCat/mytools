...
| 场景 ID | 操作与通过标准 |
| --- | --- |
| `PANEL-RESIZE-001` 主面板缩放 | 分别打开设置、剪贴板和翻译。可见玻璃首次打开为 `720 × 480 pt`（含透明外沿的窗口为 `732 × 492 pt`）；从四条直边中点、四个圆角的可见边缘分别拖动，先向外放大再向内缩小。无约束时拖动 `80 pt`，对应宽或高变化应为 `80 ± 2 pt`，对边位置变化不超过 `2 pt`；角部拖动须同时改变宽高。继续缩小可见玻璃不得低于 `600 × 414 pt`（窗口为 `612 × 426 pt`）。松手后移动鼠标，尺寸不得继续变化；关闭重开保持尺寸。 |
| `PANEL-EDGE-HIT-001` 边缘防穿透 | 将设置、剪贴板、翻译分别覆盖在仅含合成内容的其他应用窗口上。从四条直边最外侧、可见玻璃外侧 `2 pt`、`4 pt` 及四个圆角周围的透明区域分别开始拖动，主面板应正常缩放，底层应用不得收到点击或获得焦点；放大、缩小后立即重复。距可见玻璃矩形边界外 `12 pt` 的点击仍应正常交给底层应用。明暗背景下均不得新增标题栏占位、灰框或矩形背板。 |
...
| 场景 ID | 操作与通过标准 |
| --- | --- |
| `CAPTURE-POPUP-001` 临时弹窗保留 | 已授予屏幕录制权限时打开系统电池或 Wi-Fi 弹窗，按 `Option + 3` 后拖动框选弹窗、标注并复制；选区预览与剪贴板 PNG 均保留框选前的弹窗。分别在明暗背景和偏移的 Retina 显示器重复。准备期间及框选期间按 Escape 应取消且无输出；重开必须采集新画面。相邻负例：切换录屏并选择区域后应恢复实时画面，输出视频须包含后续变化；首次权限弹窗不保证保留授权前画面。 |
| `CAPTURE-TEXT-IME-001` 中文组合输入与换行 | 在明暗截图背景上分别选择 `12/16/24 pt`，输入 `3333333` 后通过中文输入法连续输入 `你好你`、`好中文输入`、`继续显示`。候选组合态和确认后均须完整显示，框宽随内容增长。按 Return 后即使尚未输入下一字，空行与光标也须可见；继续输入第二、第三行中文并连续换行，框高须随行数增长。点击框外提交，再次编辑及复制 PNG 后内容应一致；方向键、删除、撤销和 Escape 仍按原约定工作。相邻负例：普通单行文本不应多出空行，标签仍保持单行 Return 提交。分别覆盖 1× 和 Retina 缩放；记录输入法名称与版本。 |
| `CAPTURE-TEXT-MIXED-SCALE-001` 混排提交与导出一致 | 在 1× 和 Retina 的明暗背景上，以 `12/16/24 pt` 输入 `你好你好你好 hello 你好`，同时覆盖去掉英文两侧空格的变体。确认候选词、点击框外提交、双击重新编辑、修改字号后，末尾“好”均完整显示，短句不应折行，文字宽度不跳变。复制 PNG 后与预览比较末字位置；增加内容达到画布边界时，预览与 PNG 的折行位置应一致；手动回车及末尾空行仍保留。 |

`CAPTURE-TEXT-IME-001` 另须从空框连续输入 `你好啊小明`，在拼音组合、选词确认及紧接着输入下一段时检查：未达到截图宽度上限且未手动换行时，文字和光标始终保持同一行，不得先折到下一行再跳回；输入超过画布宽度的长段落仍须正常折行并完整显示。

文字排版自动化命令：`swift test --filter 'ScreenshotAnnotationTests|ScreenshotEditorInteractionTests|ScreenshotRendererTests'`；完整测试使用下文统一命令。保留修复前 Return 后高度不变、修复后空行高度增长，以及组合输入在异步扩框前提前折行、修复后保持同一行的失败与通过证据。原生文本视图测试须在组合文字更新后、主循环推进前查询光标位置，并覆盖连续选词、手动换行和画布边界自动折行；不替代真实输入法候选窗与打包应用实测。

2026-09-26 本地验证记录：在 macOS 26.5.2 的独立 AppKit/TextKit 2 合成窗口中，先将编辑视图设为 48 pt 宽，再调用 `setMarkedText` 输入拼音和五字中文，立即通过 `firstRect` 比较首字与光标行位置。12/16/24 pt 共六组在容器跟随旧框宽时全部提前折行，改用画布宽度后全部保持同一行。该实验只验证排版时序；本机 SwiftPM 因 `PackageDescription` 链接不匹配无法执行项目测试，正式包缺少受信任签名身份，开发包构建同样受工具链阻断，尚未完成打包应用的真实输入法、明暗背景和 PNG 验证。

同日混排回归补充：原生 TextKit 2 使用上述混排短句及两种空格变体，覆盖三种字号和 1×/2× 共 18 组；旧算法在 Retina 的 12/16 pt 共 6 组产生两行布局，但视图仍只有一行高。改为先按显示点字号测量、再映射到截图像素后，18 组均单行完整显示。CoreText 位图实验同时比较导出末字与显示字号字形的右边界：旧算法的三组 Retina 结果偏差超出抗锯齿容差，新算法六组均通过。该原生实验不替代打包应用真实输入法实测。
...
| `SYNC-IDLE-PERF-001` 空闲开销 | 临时目录设置仅同步收藏和置顶，普通复制后连续三轮 manifest 不变，扩大范围后仍能发布；注入外机冲突，后续周期不反复全盘扫描，解除冲突可恢复。准备四十个有台账的本机待回收目录，稳定周期分批清空且不推进 revision，无台账目录保留。容量索引在新增、原子替换、删除、审计到期后与完整盘点一致。打包应用预热后关闭面板，记录至少六十秒 CPU 增量与调用栈；区分启动盘点、定期审计和稳定周期，保留同机前后对照。 |
...
边缘防穿透需要验证 WindowServer 选择的接收窗口：可用合成前后窗口调用 `NSWindow.windowNumber(at:belowWindowWithWindowNumber:)`，在每个目标点断言返回前景面板，在远离窗口的负例点断言返回底层窗口，再用真实鼠标复核。对照结果应包含缩放前后、隐藏重开、重绘以及可见边缘外侧 `2 pt`、`4 pt` 的位置。透明外沿保持完全透明；同时比较显式设置 `ignoresMouseEvents = false` 与只依赖默认值的结果，不能用该属性的 getter 断言替代窗口级命中。该 API 按鼠标按下规则忽略透明像素；仅直接调用 `sendEvent`、`hitTest` 或设置 `contentShape` 不能证明窗口不会穿透。该验证单独在打包应用和受控合成窗口中执行，不让常规单元测试依赖当前桌面。参考 [Apple 窗口级命中规则](https://developer.apple.com/documentation/appkit/nswindow/windownumber(at:belowwindowwithwindownumber:))。

主面板保留 `.borderless`，可见玻璃和背板仅在内层 AppKit 容器按 `40 pt` 连续圆角裁切，外层承载 `6 pt` 透明拖动余量；捕获的透明角落应执行角部缩放，内容区继续保留正常操作。不得通过恢复 `.titled`、增加低透明度背板或绘制边框取得命中。无标题窗口外侧缩放的系统限制可参考 [Chromium 的窗口实现](https://chromium.googlesource.com/chromium/src/+/fb13d81be5125d0f845515455b229da97b4c760b/components/remote_cocoa/app_shim/native_widget_mac_nswindow.mm#632)；其中私有 API 不属于项目实现。
...
- Confirm the dimmed selector shows one top-centered Liquid Glass `截图 / 录屏` mode control, defaults to a blue-selected `截图`, and moves to the display where selection begins.
...
- Run `CAPTURE-POPUP-001` to verify frozen popup preservation, cancellation, a fresh frame on reopening, and live recording.
...
- Confirm the in-place editor uses one compact Liquid Glass toolbar row: annotation tools are icon-first, the selected tool is blue with an underline, drawing color and line width open mutually exclusive popovers, text size replaces line width for `文本` and `标签`, `取消` is visually secondary, and `完成` is the blue primary action. On a narrow display, confirm the toolbar remains a single unclipped row; below `476 pt`, the five drawing tools collapse into one menu while `文本` and `标签` remain directly available.
- Click near the visible rounded edges rather than directly on the icon or text for the `截图 / 录屏` mode buttons, annotation tools, undo, color and line-width controls, translation `复制译文`, settings categories, clipboard categories, and super-right-click actions; confirm the full visible button surface triggers exactly once.
- In the in-place editor, draw a line, a closed or multi-turn freehand stroke, an arrow, a rectangle, and a mosaic region. Release one freehand stroke just outside the image and confirm the accumulated in-bounds path remains. Confirm the color popover contains only the eight presets and the line-width popover contains the curved thin/standard/thick samples; both must change the live annotation and the PNG copied by `完成`. Select a non-default tool, color, and line width, close and reopen capture, and confirm all three choices remain selected. Confirm mosaic shows a boundary only while dragging, then keeps the pixelation without a colored frame; the copied PNG must retain only the pixelation.
- Select `文本`, choose each `12/16/24 pt` size, click inside a region at least `48 × 32 pt`, and confirm the empty caret and all four characters of the `输入文本` placeholder remain vertically centered and left-aligned. In the same object, enter the exact text `输入文本`, then click outside to commit; confirm the placeholder, actual input, and committed preview keep the same visible glyph size, baseline, and starting position without a first-character or commit-time jump. Confirm all three states use the selected annotation color, with only the placeholder shown at lower opacity, and both editing states keep `8 pt` of space from the blue object frame. Click the blank padding above, below, and to the left of the centered text line and confirm editing and focus remain active. Enter short text such as `333`, then click outside to commit; confirm the selection frame hugs the glyphs instead of retaining a fixed wide box. Add multiple lines and confirm the frame follows the longest line and actual line count, wrapping only at the screenshot edge. With a Chinese IME, enter `不该，不该，` and keep `333` in the active composition; confirm the editing frame grows before composition is confirmed and all characters stay visible on one line. Select `标签` in a region large enough for the current `12/16/24 pt` size (current-system minimum widths approximately `72/74/98 pt`, minimum heights `33/44/66 pt`), enter one line, and press Return to commit. Confirm both tools restore the last selected size, use the eight-color palette, and show a nearby compact object inspector after selection. Confirm text color changes the glyphs, while label color changes only the locator dot; the label bubble uses a warm-white translucent surface with readable dark text on both light and dark screenshots. Type short and long mixed Chinese/English labels, including an active IME composition, and confirm the bubble grows with the complete text without premature ellipsis or default field indentation, while the text remains horizontally and vertically centered; only content reaching the screenshot's available width truncates visually. Confirm either the inspector flip action or a direct click on the locator moves the bubble to the other side without moving the dot when that side has enough room. Place a label against an edge and confirm an impossible explicit flip keeps the locator and bubble unchanged while showing a space warning. In smaller regions, confirm the unavailable text or label tool is disabled and explains the minimum size instead of starting an invalid draft.
- Single-click an existing text or label while either text tool is active, drag it against every screenshot edge, and confirm the complete object stays inside the image and one Cmd+Z restores the whole move. Increase the font size near an edge and confirm the top of text remains anchored while the frame grows downward when space permits; confirm labels flip or shift to remain visible. Double-click an existing object, clear its content, and click outside; confirm the original object remains unchanged. During Chinese input composition, press Escape and confirm the IME candidate or marked text is dismissed first; press Escape again to discard the editor draft, then again to clear selection, and only the next Escape cancels the screenshot session. Confirm Delete removes the selected object, Cmd+Z restores creation/style/move/delete as one transaction each, Cmd+Z edits native typing while a field is focused, and Cmd+Return copies the PNG only after text editing has ended. Paste the copied PNG into Preview and confirm the text/label geometry, wrapping, truncation, colors, and locator direction match the editor without selection outlines or the inspector.
- Switch the top control to `录屏` and drag a region; confirm the frozen selection background disappears and recording captures live changes, the compact floating Liquid Glass control is pinned at the top center of the selected display, shows one red recording dot, `录制`, monospaced elapsed time, and a labeled red `停止` action without unused trailing space, and does not appear in the recorded video.
...
- Connect displays on the left, right, top, and bottom when available. Put a window on the matching edge, repeat the same directional shortcut, and confirm it moves to the adjacent display's inner edge: left enters its right side, right enters its left side, top enters its bottom side, and bottom enters its top side. Confirm one-third and two-thirds layouts preserve their fraction while flipping edge, and a direction with no neighboring display stays on the current display without wrapping.
...
- Open `通用` at `980 × 900` and `640 × 520`; confirm `系统`, `软件更新`, and `快捷键` each use one rounded grouped surface, the `登录时自动启动` and update switches use the small macOS control size in compact rows, the `打开 ↗` action is a compact neutral capsule, labels and trailing controls share stable alignment, compact mode remains scrollable, and no divider, hover surface, rectangular backing, or shadow escapes the card corners in light or dark appearance.
- In the packaged and signed app, enable `通用 > 系统 > 登录时自动启动`; confirm macOS lists MacTools under `通用 > 登录项与扩展`, the switch stays enabled after relaunching MacTools, and MacTools starts after the next login. Disable it and confirm the login item is removed without terminating the running app. If macOS reports that approval is required, confirm the settings row shows `打开登录项`, opens the correct System Settings pane, and refreshes immediately after approval or denial when MacTools becomes active.
- Open `通用 > 软件更新` and confirm the section shows only the user-facing installed version, without the internal build number, alongside one `检查更新…` button, `自动检查更新`, and `自动下载并安装更新`. Confirm the automatic-download toggle is disabled whenever automatic checks are off and that both choices persist after relaunching the packaged app.
- Click `检查更新…` from the packaged app and confirm the button disables and shows progress while Sparkle is checking. Confirm Sparkle's checking, result, download, installation, and error prompts use Simplified Chinese. On the latest release or a higher semantic local package, confirm Sparkle reports that the app is up to date instead of offering an older stable release. Build and install an intentionally lower stable version through `MACOS_SIGNING_MODE=stable MACOS_APP_VERSION=<lower> scripts/rebuild_and_run_app.sh`, then confirm it discovers the current stable GitHub Release and presents the signed update flow.
- Package with `MACOS_SIGNING_MODE=development scripts/package_app.sh`; confirm `build/MacTools Dev.app` contains `Contents/Frameworks/Sparkle.framework`, all Sparkle nested executables pass code-sign verification, and `Info.plist` contains neither `SUFeedURL` nor `SUPublicEDKey`, with automatic checks and downloads disabled. Launch it and confirm the software-update section stays disabled without showing an updater startup error or installing a production release.
- For a tagged release, download the DMG, SHA-256 file, and `appcast.xml` from GitHub Release. Confirm the checksum passes, the appcast XML and feed signature validate, `releases/latest/download/appcast.xml` is byte-identical to the release asset, and a lower stable packaged build can discover and install the release. Confirm the canonical `/Applications/MacTools.app` version/build and fixed anonymous certificate change to the release while Accessibility, Input Monitoring, and automatic paste remain granted under the same formal MacTools entry.
...
- Return from each Privacy & Security pane and confirm the permission status refreshes as soon as MacTools becomes active, without closing Settings.
- From an old ad-hoc installation, click `整理旧权限记录`, cancel once and confirm no permission changes. Confirm the action, verify all TCC decisions for `local.mactools.mvp` are cleared while unrelated applications remain unchanged, and re-grant Accessibility, Input Monitoring, automatic paste, Screen Recording, and Finder Automation. Remove any remaining raw-executable legacy entries with the System Settings minus button and confirm only one formal MacTools entry remains per pane.
- Build with `MACOS_SIGNING_MODE=development scripts/package_app.sh`; confirm `build/MacTools Dev.app` requires no CloudKit/iCloud Container/APNs entitlement or Provisioning Profile and can choose a sync folder only while the formal app is not running.
...
- Replace development build A with a separately packaged development build B, then quit and relaunch build B again. Confirm neither launch requests Keychain authorization and both restore the saved credential from the local encrypted cache.
...
- Build the stable app bundle with `MACOS_SIGNING_MODE=stable scripts/package_app.sh`.
- Inspect the stable bundle with `codesign -dv --verbose=4` and `codesign -d -r-`; confirm the only authority is `MacTools Release Signing`, the leaf SHA-1 is `25A3263958804C6D9429EB51B97BA2B16CA1FB67`, the designated requirement combines that certificate with `local.mactools.mvp`, and no name, email, Apple Team, Apple Development, or Developer ID field appears.
- Push the reviewed commit to `codex/anonymous-signing-preflight`; confirm the owner-only workflow derives the P-256 identity from the existing `SPARKLE_PRIVATE_KEY`, builds and verifies a stable bundle, cleans the temporary private key/PKCS#12/Keychain/trust, and neither creates a tag nor publishes a Release.
- Install and launch `/Applications/MacTools.app` through `scripts/rebuild_and_run_app.sh`.
