...
| 翻译 | 在线提供方为阿里云百炼，提供方 ID 为 `bailian`，模型为 `qwen-mt-flash`，使用 DashScope 的 OpenAI 兼容端点；主翻译面板和超级右键译文均支持系统语音朗读 |
...
- 所有版本判断、标签创建、工作流等待和发布后核验统一遵循 `docs/release-guide.md`；该文件是唯一发布步骤清单，不在其他文档复制另一套流程。
- 正式发布入口只有一个：在已验证且已推送的 `main` 提交上创建并推送带注释的 `vX.Y.Z` 标签，由 `.github/workflows/release.yml` 自动生成 GitHub Release。
- 日常发布不得自行增加发布指南未列出的步骤。匿名签名预检、低版本真实更新、业务场景人工验证只在发布指南规定的条件命中时执行；需要临时增加检查时，必须先说明原因、预期收益和耗时。
- 标签推送后不得删除、移动或覆盖标签；工作流失败时保留标签和失败证据，重新评估修复方案与下一个版本。
...
- 用户 Bug 修复须关联 [人工验证清单](docs/manual-verification.md) 的稳定场景 ID，并按其中的触发关系、完整测试命令与证据要求验证，保留修复前失败和修复后通过的依据。
...
| 软件更新或发布 | 按 `docs/release-guide.md` 的条件验证表执行；只有更新、签名或发布链路发生变化时，才要求较低版本真实更新验证 |
