...
MacTools 是一个使用 Swift Package Manager 构建、面向 macOS 26 及以上版本的菜单栏应用。已实现的功能包括剪贴板历史、百炼翻译、超级右键操作、区域截图与屏幕录制、窗口布局、设置和权限诊断。应用采用辅助应用激活策略运行，默认不显示 Dock 图标。
...
| 翻译 | 在线提供方为阿里云百炼，提供方 ID 为 `bailian`，模型为 `qwen-mt-turbo`，使用 DashScope 的 OpenAI 兼容端点 |
