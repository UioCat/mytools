// 应用运行时的依赖装配与功能协调中心。
// 负责连接 Core 服务、功能状态和平台适配器，不定义可复用的领域规则。
...
final class AppEnvironment {
    ...
    /// 使用 Accessibility 适配器把指定布局应用到当前聚焦窗口，失败只记录日志。
    ...
    /// 持久化窗口布局按钮和快捷键配置，并通知 AppDelegate 重新注册热键。
    ...
    /// 尝试持久化截图编辑设置；失败记录日志并让编辑器保留原已保存状态。
    ...
    /// 原子保存账号级同步设置和设备级开关，并按成功结果启停同步协调器。
    ...
    /// 将异步解析的 Finder 当前目录转换为临时文件夹条目；解析失败时降级为窗口布局面板。
